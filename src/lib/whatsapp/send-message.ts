...
import { sendMetaMessageToConversation } from '@/lib/meta/send-message';
...
export async function sendMessageToConversation(
  ...
  // Messenger / Instagram hand off here. Everything below this line —
  // phone sanitisation, whatsapp_config, templates, interactive — is
  // WhatsApp-specific and would either throw or send nonsense on a
  // Meta conversation. `channel` defaults to 'whatsapp' (migration
  // 040), so existing rows never take this branch.
  if (
    conversation.channel === 'messenger' ||
    conversation.channel === 'instagram'
  ) {
    const result = await sendMetaMessageToConversation(
      db,
      accountId,
      conversation.channel,
      { conversationId, messageType, contentText, mediaUrl },
    );
    // Reuse the existing result shape so callers (the dashboard route
    // and the public v1 endpoint) need no changes — the field is named
    // for WhatsApp but carries whichever provider id applies.
    return {
      messageId: result.messageId,
      whatsappMessageId: result.metaMessageId,
    };
  }
