...
interface NavItem {
  ...
  beta?: boolean;
  /**
   * Placeholder row: renders dimmed, with a "Soon" chip, and does not
   * navigate. The route doesn't exist yet, so we deliberately skip the
   * <Link> wrapper — a dead href would 404 instead of reading as
   * "coming soon".
   */
  soon?: boolean;
  /** Minimum role for the row to exist at all. Absent = everyone sees it. */
  minRole?: AccountRole;
}
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          <ul className="flex flex-col gap-1">
            {navItems
              // Fail closed: while the role is still resolving we don't
              // know it yet, so gated rows stay hidden. Prevents a
              // flash of "Analytics" for an agent on first paint.
              .filter(
                (item) =>
                  !item.minRole ||
              ...
              // "Coming soon" row — same box model as a real row so the
              // list height doesn't shift when the route ships, but
              // inert and skipped by keyboard navigation.
              if (item.soon) {
                return (
                  <li key={item.href}>
                    <div
                      aria-disabled="true"
                      className="flex cursor-default items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-muted-foreground/60 lg:py-2"
                    >
